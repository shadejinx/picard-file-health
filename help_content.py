...
separate results, and one never blocks or requires the other — matching a
...
against published transparency thresholds. No sensitivity sliders — a file
either has a real structural defect or it doesn't, so there's nothing to
tune.</p>
...
&mdash; this is the only scan available on unmatched files, since it doesn't
need a matched track to run.</p>
<p><b>File Health tiers</b>, worst to best:</p>
<ul>
<li><b>Unplayable</b> &mdash; ffmpeg couldn't decode it at all (corrupt,
truncated, unsupported, or timed out).</li>
<li><b>Bad</b> &mdash; decodes, but has a real structural defect: an audio
corruption signature, a file-size mismatch against its own header, corrupt
embedded artwork, or a malformed tag structure.</li>
<li><b>OK</b> / <b>Good</b> / <b>Excellent</b> &mdash; structurally sound,
...
the Checks &amp; Why tab) &mdash; at or above it is Good, below it is OK.</li>
</ul>
...
channels, background noise floor, and dynamic range. Always manual &mdash;
right-click a matched track and choose <b>Scan Track Health...</b>, or use
the buttons in the Details window &mdash; never automatic, since it's
meaningfully slower than File Health.</p>
<p><b>Track Health tiers</b>: <b>Bad</b>, <b>OK</b>, <b>Good</b>,
<b>Excellent</b>. There's no "Unplayable" tier here &mdash; if the file
won't decode, that's already the File Health verdict, and Track Health
simply has nothing to measure.</p>
...
one composite score. A single borderline reading &mdash; True Peak just a
fraction of a dB over its threshold, say &mdash; can't push a file to
"Bad" on its own; that verdict is reserved for files where several checks
agree, or one is badly wrong. The sliders under "Track Health Sensitivity"
above each move their check's own threshold directly, which reshapes the
composite score in real time on the next scan.</p>
"""
...
&mdash; corrupt, truncated, unsupported, or timed out. Forces
Unplayable.</li>
...
decodes to an impossible value &mdash; a genuine internal inconsistency,
not a guess. Validated against 600 real files spanning five real
...
ID3 tag wrapper). A mismatch beyond 5% is a real structural fact &mdash;
independent of any decode &mdash; that the file was altered after
encoding: data appended after the original stream ended, or the file cut
...
"generally transparent" &mdash; MP3 at/above 192kbps, plain AAC-LC at/above
...
<ul>
<li><b>Clipping (CLP).</b> astats' "Flat factor" &mdash; runs of identical
...
peak measurement &mdash; catches reconstruction overshoot that can clip on
playback even when no single sample is at full scale, which Flat factor
alone can't see. Default threshold: +0.6dBTP, deliberately above the
theoretical 0dBTP ceiling to stay outside that same standard's own
documented ~0.554dB measurement margin at 4x oversampling &mdash; so a
...
("fake hi-res" &mdash; genuine hi-res content extends past 24kHz). When
the file carries a LAME encoder header, the check additionally confirms
against that encoder's own recorded low-pass setting for decisive, not
just heuristic, evidence that the missing content isn't this encode's own
doing. Fake Hi-Res shares this same slider &mdash; it has no separate
control of its own.</li>
...
there &mdash; hiss, static, mastering-chain self-noise &mdash; that
persists even once the music itself has dropped out is real background
noise. Default threshold: -35dB, calibrated against this project's own
real-track sample (measured noise floors ranged -66.4dB to -37.5dB).</li>
...
green at DR14+). Unlike the other checks, this isn't pass/fail &mdash; it's
a continuous gradient that shifts the score smoothly as compression gets
heavier, and its one slider shifts every band together rather than exposing
each boundary separately.</li>
</ul>
...
&mdash; direct, high-confidence measurements of the decoded signal. True
...
with full certainty from legitimate content &mdash; room tone, a reverb
tail, a sustained musical drone at the same pitch.</p>
...
frequency (50Hz or 60Hz) found specifically inside quiet passages &mdash;
never checked across the whole file, precisely because a real musical
note at the same pitch would stop when the rest of the music does, while
hum doesn't. Flagged when a narrowband spike measures 8dB or more above a
nearby control frequency.</li>
<li><b>Mono content in a stereo container.</b> Left and right channels are
identical &mdash; not a defect, just a note that no unique stereo
...
left/right channels are. Both are cause-agnostic &mdash; a naturally
treble-light acoustic recording or an intentionally wide mix reads
identically to a real defect &mdash; so Spectral Cutoff and Out-of-Phase
remain the actual gates; these are just supporting context.</li>
...
<h2>Layout</h2>
<p>One row per file, grouped by the track Picard has matched it to &mdash;
so duplicate copies of the same recording line up side by side for direct
...
perceptual check &mdash; each tag matches the sensitivity slider that
controls it on the Options page.</td></tr>
...
<ul>
<li>&#9989; <b>Green</b> &mdash; comfortably clear of the threshold.</li>
<li>&#9888;&#65039; <b>Amber</b> &mdash; passes right now, but one slider
...
<li>&#10060; <b>Red</b> &mdash; fails; past the threshold.</li>
<li>&#10134; <b>Gray</b> &mdash; not applicable (for example, a mono file
...
you &mdash; the numbers are shown side by side, the choice is yours.</p>

<h2>Buttons</h2>
<ul>
<li><b>Scan File Health / Scan Track Health</b> &mdash; scans every file
currently listed in the window.</li>
<li><b>Show in List</b> &mdash; selects and scrolls to the selected file in
Picard's own main window (useful once files are matched and the main list
shows track titles instead of filenames).</li>
<li><b>Spectrogram...</b> &mdash; renders a visual spectrogram for the
selected file. See the next tab.</li>
<li><b>Remove from Picard</b> / <b>Move to Trash...</b> &mdash; act on the
selected file directly, without needing to find it in the main window
first.</li>
</ul>
<p>The window stays open and redraws in place as scans finish &mdash; no
...
picture of the same audio the numeric checks already measured &mdash; a
way to visually double-check a flagged (or clean) file rather than
trusting a single number alone.</p>
...
unchanging over time</b> &mdash; a spectral cutoff wall. This is the
classic visual signature of a prior lossy transcode, or an upsampled
"fake hi-res" file &mdash; exactly what the Spectral Cutoff / Fake Hi-Res
checks measure numerically.</li>
<li><b>A persistent haze or glow along the bottom during otherwise-quiet
passages</b>, instead of clean black &mdash; an elevated noise floor
...
instant</b> &mdash; a broadband click or pop, a likely sign of localized
...
other's</b> &mdash; a channel-specific problem: a bad transfer, a damaged
mic or cable at recording time, or a mono source improperly widened.</li>
<li><b>No sharp cutoff, an even noise floor, and roughly matching
left/right bands</b> &mdash; nothing visually wrong, matching what a clean
...
&mdash; it doesn't replace them. Some things &mdash; a precise True Peak
reading, the exact DR14 rating, the exact Out-of-Phase angle &mdash;
aren't reliably eyeballed from a picture. Use the Details window's own
numbers and tooltips for anything you need to act on with confidence; use
the spectrogram to sanity-check what those numbers are telling you.</p>
"""
