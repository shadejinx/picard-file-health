...
`find_ffmpeg` resolves the binary in three tiers, most-authoritative first: an explicit user-configured path (Options page) is validated as an existing, executable file (rejecting anything else, closing off "configure an arbitrary non-executable path" as a way to redirect what gets run) and always wins even when a fallback below would also succeed. Otherwise `shutil.which` searches `PATH` — sufficient on macOS/Linux, where Homebrew/apt installs both add ffmpeg to `PATH` automatically. Only when `PATH` search itself fails does a third tier apply, and only on Windows: a fixed, ordered fallback search covering the ways Windows users commonly end up with ffmpeg installed without it landing on `PATH`:

1. WinGet's own shim directory, `%LOCALAPPDATA%\Microsoft\WinGet\Links\ffmpeg.exe`.
2. WinGet's per-package installation directories under `%LOCALAPPDATA%\Microsoft\WinGet\Packages\`, for the two ffmpeg packages the community publishes — `Gyan.FFmpeg` searched before `BtbN.FFmpeg.GPL` when both are present, an arbitrary but stable tie-break (`Gyan.FFmpeg` is the more widely recommended of the two). Both install as an extracted, version-suffixed archive rather than a registered portable command, so WinGet's own `PATH` registration for them is unreliable in practice — a known WinGet packaging defect: the `PATH` entry WinGet adds for `Gyan.FFmpeg` points at the wrong subfolder ([microsoft/winget-pkgs#95349](https://github.com/microsoft/winget-pkgs/issues/95349)). Matched by globbing the package-ID-prefixed directory for a nested `bin\ffmpeg.exe`, since the version-numbered build folder inside changes with every release; when more than one such build folder exists (an upgrade that left an older extraction behind), the newest by directory modification time wins.
3. Conventional manual-install/installer locations, read from `%ProgramFiles%`, `%ProgramFiles(x86)%`, and `%SystemDrive%` (each always set on a real Windows install; defaulting to the standard `C:\` locations if absent, for defensive robustness rather than as the expected path): `<ProgramFiles>\ffmpeg\bin\ffmpeg.exe`, `<ProgramFiles(x86)>\ffmpeg\bin\ffmpeg.exe`, `<SystemDrive>\ffmpeg\bin\ffmpeg.exe`.

Any environment variable this search depends on (`LOCALAPPDATA`, or the tier-3 variables above) that isn't set is treated as "this tier has nothing to check," not an error — the search degrades silently through unavailable tiers, per the HLD's graceful-degradation tenet. This fallback only ever runs on Windows (`sys.platform == 'win32'`, the same check `_SUBPROCESS_KWARGS`'s console-suppression logic already uses) and only after `PATH` search has already failed; macOS/Linux and any Windows install already on `PATH` are unaffected. It runs on every `find_ffmpeg(None)` call with no caching or persisted state — kept a pure resolution function, consistent with the rest of its contract.

`ffprobe` is looked for alongside whatever `ffmpeg` binary was resolved (same directory, matching every mainstream distribution's layout — including every fallback tier above, since each resolves to a real installation's own `bin` directory), falling back to a bare `PATH` search. `_find_ffprobe` needs no Windows-specific fallback of its own: it already derives ffprobe's expected location from `find_ffmpeg`'s result, fallback-found or not.

## ffmpeg Version Requirements

A minimum ffmpeg version (3.1) is enforced — the floor `astats`' `metadata`/`reset` options need. A version string that doesn't parse (some git/dev-snapshot builds report a non-numeric string like `N-12345-gabcdef` instead of a release number) is treated as "let it through" rather than a failure: those builds are essentially always newer than any numbered release, so blocking on an unparseable string would reject far more legitimate newer installs than it would ever catch a genuinely too-old one.
...
| Windows ffmpeg fallback search | Fixed ordered list (WinGet Links → WinGet Packages glob → conventional install dirs), only after `PATH` search fails, no caching | Always search fallback dirs first; prompt the user to configure the path instead; persist a found path back into the Options page field | `PATH` is the platform-standard signal and should win when present; searching fallback dirs unconditionally would risk picking a stale/wrong install over a deliberately-`PATH`'d one. Prompting instead of auto-discovering was rejected — the two WinGet-published ffmpeg packages are popular precisely because they're the easiest "just get ffmpeg" path for a non-technical Windows user, and WinGet's own known PATH bug means that easy path silently fails today. Not caching the result keeps `find_ffmpeg` a pure resolution function with no side effects, consistent with its existing contract; the filesystem glob this adds is cheap enough (a handful of `Path.glob` calls, at most once per scan) not to need memoizing. |
