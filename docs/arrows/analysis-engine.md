...
2. ffmpeg version/path resolution (`find_ffmpeg`, `check_ffmpeg_version`) — PATH search, explicit-path validation, version-string parsing with a graceful "unparseable → accept" fallback.
...
| ffmpeg Version Resolution | VERSION-001 to 004 | 4 | 0 | 0 |
