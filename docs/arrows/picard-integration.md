...
**OK** — fully coherent as of 2026-09-06 (git SHA `PLACEHOLDER`). All 26 specs implemented and annotated at their code entry point; every spec has at least one test citing it (35 tests total, some specs covered by more than one). No coverage gaps, no orphan or reverse-orphan spec IDs found.
...
### EARS
- `docs/intent/picard-integration/picard-integration-specs.md` (26 specs: `UI-COL-*` ×5, `UI-ACTION-*` ×7, `UI-OPTIONS-*` ×2, `UI-DETAILS-*` ×4, `UI-SPECTRO-*` ×2, `UI-HELP-*` ×2, `UI-META-*` ×2, `UI-SEC-*` ×2)

### Tests
- `tests/test_picard_integration.py` (35 tests, using a duck-typed Qt/Picard harness — see Key Findings)
...
2. Five actions: File Health scan (everywhere), Track Health scan and the Details launcher (matched content only, reachable via a Track node or a linked File node — see Key Findings for why file-action registration was required), Details window (one file, or "all files" from the Tools menu) — every scan on a background thread via `run_task`, with pending-state and error-status-bar handling for a file removed from every visible tree mid-scan.
...
| Actions | ACTION-001 to 007 | 7 | 0 | 0 |
...
**Summary:** 26 of 26 active specs implemented; 0 deferred; 0 gaps.
...
9. **`UI-ACTION-002` reworked and `UI-ACTION-007` added this session — Track Health/Details actions were unreachable for a common real case.** A user report (right-click Track Health/Details options missing for a track with multiple matched files) traced to Picard's own context-menu builder: it derives available plugin actions purely from the type of the single item under the cursor, and Picard renders a track with more than one linked file as a Track parent with the linked files as separate File child nodes — not as one Track node. Track-only registration meant right-clicking those File children (the only way to reach that content) never showed Track Health or Details. Fixed by also registering both actions as file actions; `ScanTrackHealthAction.callback` now filters to `isinstance(file.parent_item, Track)` so the actual scope — matched content only — is unchanged, only where it's reachable from.
